...
(function (E) {
  ...
  // one-level-at-a-time loop (pinned by a randomized test against
  // E.buy(s, i, 1)); it is just cheaper:
  ...
  function autoBuy(s, u, m, cache) {
    ...
    while (left > 0) {
      ...
      // relative precision (subtracting from a tiny rem loses digits).
      if (rem < 1e-3) { base = rem > 0 ? base + Math.log10(rem) : -Infinity; rem = 1; if (base === -Infinity) break; }
      did = true;
